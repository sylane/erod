...
perform_action(get_content, _, #?Ctx{policy = undefined} = Ctx) ->
    ...
    erod_context:failed({get_content_error, not_authenticated}, Ctx);
...
perform_action(get_children, _, #?Ctx{policy = undefined} = Ctx) ->
    ...
    erod_context:failed({get_children_error, not_authenticated}, Ctx);
...
perform_action(login, _, Ctx) ->
    ...
    erod_context:failed({login_error, already_authenticated}, Ctx);
    ...
    try
        case erod_session_manager:find_session(Token) of
            {error, Reason} ->
                ...
                erod_context:failed({restore_error, Reason}, Ctx);
            {ok, Session} ->
...
perform_action(restore, _, Ctx) ->
    ...
    erod_context:failed({restore_error, already_authenticated}, Ctx);

perform_action(bind, _, #?Ctx{sess = undefined} = Ctx) ->
    erod_context:warning("Cannot bind, not yet authenticated.", [], Ctx),
    erod_context:failed({bind_error, not_authenticated}, Ctx);
...
perform_action(logout, _, #?Ctx{user = undefined} = Ctx) ->
    ...
    erod_context:failed({logout_error, not_authenticated}, Ctx);

perform_action(logout, Args, #?Ctx{user = User} = Ctx) ->
    erod_user:perform(User, logout, Args, Ctx);

perform_action(Action, Args, Ctx) ->
    erod_context:error("Cannot perform unexpected action ~p with arguments ~p.",
                       [Action, Args], Ctx),
    erod_context:failed({internal_error, unexpected_action}, Ctx).
